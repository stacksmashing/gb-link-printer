        ...
        if (navigator.serial) {
            if (this.state.arcade === this.ArcStateConnect) {
                ...
                if (canvas && this.state.printdata && this.state.linesCount > 0) {
                    ...
                    console.log("Palette: %o", actualPalette);
                    console.log("Margins: %o", this.margins);

                    // Resize canvas
                    console.log("%d bytes to print...", this.state.printdata.length);
                    console.log("%d tiles to print...", (this.state.printdata.length/16)); // 16 bytes per tile
                    //linesCount = ((this.printdata.length/16)/20);
                    console.log("%d lines to print...", this.state.linesCount);    // 20 tiles per line
                    canvas.height = 8 * ( this.state.linesCount + this.margins.top + this.margins.bottom);
                    ...
                    // Re-render with current palette from raw link data
                    for(let y = 0; y < this.state.linesCount; y++) {
